...
interface LogFormProps {
  loading: boolean
  dataTransfer: (data: { SelectData: number }[]) => void;
  optionTransfer: (data: { DroneId: string; SensorData: string }[]) => void;
  setLoading: any
}

const TFTPredictionForm: React.FC<LogFormProps> = (props:any) => {
  ...
  return (
    <div id='predictiondropdown' className={classNames}>
      <span className='log-form-header-text'>
        {themeName === 'tft' ? <img src={icon} alt='icon' width={60}/> : <>•</>} 부품 및 조회기간 선택</span>
...
export default TFTPredictionForm
